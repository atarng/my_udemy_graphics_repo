...
function createStairs() {
	...
	for(var i = 0; i < 6; ++i) {
		...
		// half of height: put it above ground plane
		stepMesh.position.y = (0.5 + i) * verticalStepHeight + (i * stepThickness);
		...
		stepMesh.position.y = ((0.5 + i) * stepThickness) + ((1+i) * verticalStepHeight);
		// Push step forward by half the depth, minus half the vertical step's thickness
		// horizontalStepDepth / 2
		stepMesh.position.z = (0.5 + i) * (horizontalStepDepth) - ((i+0.5) * stepThickness);
		scene.add( stepMesh );
	}
}
