...
function createCapsule( material, radius, top, bottom, segmentsWidth, openTop, openBottom ) {
	...
	if(!openTop) {
		let top_sphere = new THREE.Mesh( sphGeom, material );
		top_sphere.position.set(top.x,top.y,top.z);
		capsule.add(top_sphere);
	}
	if(!openBottom) {
		let bot_sphere = new THREE.Mesh( sphGeom, material );
		bot_sphere.position.set(bottom.x, bottom.y, bottom.z);
		capsule.add(bot_sphere);
	}
